...
/**
 * Sets the width and height of the canvas, returns the rendering context
 * @param {number} width
 * @param {number} height
...
function new_image(width, height) {
    ...
    var canvas = $("map"); // HTMLCanvasElement
    canvas.width = width;
 ...
 * @returns {Map_Details}
 */
function scale_dungeon(dungeonOBJ) {
    var b = new Map_Details (
        dungeonOBJ.map_style,
        dungeonOBJ.grid,
        dungeonOBJ.cell_size,
        0,
        0,
        0,
        0,
    ...
    b.width = (dungeonOBJ.n_cols + 1) * b.cell_size + 1;
    b.height = (dungeonOBJ.n_rows + 1) * b.cell_size + 1;
    b.max_x = b.width - 1;
    b.max_y = b.height - 1;
 ...
 */
function get_palette(mapDetails) {
    var palette_loc = new Palette();
    if (mapDetails.palette) {
        palette_loc = mapDetails.palette;
    }
    else
    {
        var style = mapDetails.map_style;
        if (style)
        {
            if (palette[style])
            {
    ...
    //     : palette.standard;
    var c = new Palette_Details();
    if (c = palette_loc.colors) $H(c).keys().each(function(palette_detail) {
        palette_loc[palette_detail] = c[palette_detail]
    });
 ...
 * @param {Map_Details} mapDetails 
 */
function base_layer(dungeonOBJ, mapDetails, dungeon_image) {
    var c = new Element("canvas");
    c.width = mapDetails.width;
    c.height = mapDetails.height;
    ...
    if (h = palette.open_grid) image_grid(dungeonOBJ, mapDetails, h, d);
    else if (h = palette.grid) image_grid(dungeonOBJ, mapDetails, h, d);
    return c
}
 ...
 * @param {*} d 
 */
function image_grid(imageSettings, mapDetails, c, d) {
    if (imageSettings.grid != "none")
        if (imageSettings.grid == "hex") hex_grid(imageSettings, mapDetails, c, d);
        else imageSettings.grid == "vex" ? vex_grid(imageSettings, mapDetails, c, d) : square_grid(imageSettings, mapDetails, c, d);
    return true
}
 ...
 * @param {*} d 
 */
function square_grid(imageSettings, mapDetails, c, d) {
    var a = mapDetails.cell_size;
    var e;
    for (e = 0; e <= mapDetails.max_x; e += a) draw_line(d, e, 0, e, mapDetails.max_y, c);
    for (e = 0; e <= mapDetails.max_y; e += a) draw_line(d, 0, e, mapDetails.max_x, e, c);
    return true
}
 ...
 * @param {*} d 
 */
function hex_grid(imageSettings, mapDetails, c, d) {
    var e = mapDetails.cell_size / 2;
    var a =  mapDetails.cell_size / 3.4641016151;
    var widthInHexes = mapDetails.width / (3 * a);
    var heightInHexes = mapDetails.height / e;
    var x;
    for (x = 0; x < widthInHexes; x++) {
        var h = x * 3 * a,
            i = h + a,
            j = h + 3 * a,
            y;
        for (y = 0; y < heightInHexes; y++) {
            var l = y * e,
                o = l + e;
            if ((x + y) % 2 != 0) {
